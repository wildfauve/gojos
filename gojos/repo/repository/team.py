...
class TeamRepo(graphrepo.GraphRepo):
    rdf_type = rdf.TEAM
    ...
    def creator(self, team, g, sub):
        g.add((sub, RDF.type, rdf.TEAM))
        g.add((sub, rdf.skos.notation, Literal(team.name)))
        g.add((sub, rdf.hasTeamMembers, Literal(team.members)))
        [g.add((sub, rdf.hasFeature, Literal(feat.value))) for feat in team.features]
        return g
