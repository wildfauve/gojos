...
def wildcards(major):
    ...
    """
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Hovland).trade_in(players.Morikawa))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Rahm).trade_in(players.DeChambeau))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Zalatoris).trade_in(players.Davis))
    TEAM.major(major).play_wildcard(WildCard().from_round(4).trade_out(players.Lowry).trade_in(players.Smith_Ca))
    pass
