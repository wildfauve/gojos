...
def selection(major):
    ...
    TEAM.major(major).on_roster(players.Wu)
    TEAM.major(major).on_roster(players.Spieth)
    TEAM.major(major).on_roster(players.Conners)
    TEAM.major(major).on_roster(players.Scott)
...
def wildcards(major):
    ...
    """
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Rose).trade_in(players.McIlroy))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Thomas).trade_in(players.Fowler))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Spieth).trade_in(players.Kim_S_W))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(players.Scott).trade_in(players.Schauffele))
    pass
