...
def wildcards(major):
    ...
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(Hatton).trade_in(McIlroy))
    TEAM.major(major).play_wildcard(WildCard().from_round(3).trade_out(Finau).trade_in(Scheffler))
